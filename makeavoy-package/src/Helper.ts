...
function hexToRGB(h: number | string) {
  if (typeof h !== "string") {
    h = h.toString(16);
  } else {
    if (h.startsWith("#")) h = h.substring(1);
    else if (h.startsWith("0x")) h = h.substring(2);
  }
...
function hexToRGBFloat(h: number | string) {
  let ar = hexToRGB(h);
  return ar.map((v) => v / 256.0);
}
...
function rgbToBinary(r: number, g: number, b: number) {
  return parseInt(
    "0x" + componentToHex(r) + componentToHex(g) + componentToHex(b)
  );
}

function rgbFloatToHex(r: number, g: number, b: number) {
  r = Math.floor(r * 256);
  g = Math.floor(g * 256);
  b = Math.floor(b * 256);
