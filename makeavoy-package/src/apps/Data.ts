...
export default class Data extends App {
  ...
  constructor(dom) {
    ...
    input.value =
      "70.114, 69.14, 69.14, 68.653, 68.653, 69.14, 68.653, 69.627, 69.627, 68.653, 69.627, 69.14, 69.14, 68.653, 70.114, 69.14, 69.14, 69.14, 68.653, 69.14";
    ...
    button.addEventListener("click", (ev) => {
      ...
      if (array && array.length > 0) {
        ...
        this.barGraph(array, groupMove, normalize.checked, size);
      }
    });
    ...
    // TODO is it worth setting this still?
    // Render.specterMaterial.color = 0xd53229;
    let pyramid1 = new THREE.Mesh(
      new THREE.ConeGeometry(10, 10, 4),
      new THREE.MeshStandardMaterial({ color: 0x6a00fe })
    );
    pyramid1.position.set(-50, 0, 0);
    shapes.push(pyramid1);
    groupPivot.add(pyramid1);

    let pyramid2 = new THREE.Mesh(
      new THREE.ConeGeometry(10, 10, 4),
      new THREE.MeshStandardMaterial({ color: 0xd53229 })
    );
    ...
    this.barGraph(
      [
        70.114, 69.14, 69.14, 68.653, 68.653, 69.14, 68.653, 69.627, 69.627,
        68.653, 69.627, 69.14, 69.14, 68.653, 70.114, 69.14, 69.14, 69.14,
        68.653, 69.14,
      ],
      groupMove,
      true,
  ...
  barGraph(data, scene, normalize, size) {
    ...
    if (!normalize) {
      let cubeBase = new THREE.Mesh(
        geo,
        new THREE.MeshStandardMaterial({ color: lowColor })
      );
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let scaleFactor = 0.01 + val * size;
      let color = Helper.rgbToBinary(
        Math.floor(lowRGB[0] + rDelta * val),
        Math.floor(lowRGB[1] + gDelta * val),
        Math.floor(lowRGB[2] + bDelta * val)
      );
      //console.log(scale)
      //if(i==0){
